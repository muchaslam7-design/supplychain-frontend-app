  ...
  const handlePredict = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        "https://supplychain-ai-engine-production.up.railway.app/predict",
        {
          method: "POST",
          headers: {
      ...
      if (data && data.prediction) {
        ...
        console.log("Invalid response format received:", data);
        alert("Backend response error. Check console logs.");
      }
    } catch (error) {
